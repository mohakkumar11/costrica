...
$config_email = get_option( 'apb_mail_cancel' );
...
foreach ( $order_data as $item ) :
	...
	<div style="border-bottom: 1px solid #e4e4e4; padding: 20px 20px 15px;" class="apb-room-selected_item">
		...
		<div class="apb-room-seleted_name has-package">
			<h2 style="color: #1e63a0; display: block; font-family: &quot;Helvetica Neue&quot;, Helvetica, Roboto, Arial, sans-serif; font-size: 18px; font-weight: bold; line-height: 130%; margin: 16px 0 8px; text-align: left;">
		...
		<div style="border-top: 1px solid #e4e4e4; margin-top: 15px; padding-top: 5px;" class="apb-room-seleted_package">
			...
			<ul style="list-style: outside none none; margin-bottom: 0; margin-top: 5px; padding-bottom: 2px; padding-left: 0;">
				<?php
				$info_price_day = AWE_function::get_pricing_of_days( $item['from'], $item['to'], $room_type_id, 0 );
				foreach ( $info_price_day as $month => $list_day ) {
					foreach ( $list_day as $day => $price_day ) {
						...
						<li style="color: #333333; font-size: 12px; overflow: hidden;">
							<span class="apb-room-seleted_date"><?php echo esc_html( date( get_option( 'date_format' ), strtotime( $month . '/' . $day . '/' . date( 'Y' ) ) ) ); ?></span>
					...
					<li style="color: #333333; font-size: 12px; overflow: hidden;">
						...
						if ( is_array( $extra_adult ) ) {
							foreach ( $extra_adult as $item_extra_adult ) {
								if ( $item['room_adult'] == $item_extra_adult['number'] ) {
									?>
									<span class="apb-room-seleted_date"><?php printf( esc_html__( '%s Adult', 'awebooking' ), absint( $item_extra_adult['number'] ) ); ?> + <?php printf( esc_html__( '%s/night', 'awebooking' ), wp_kses_post( AWE_function::apb_price( $item_extra_adult['price'] ) ) ); ?></span>
									<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount" class="apb-amount"><?php echo wp_kses_post( AWE_function::apb_price( $item_extra_adult['price'] ) ) ?> × <?php echo absint( count( $range_date ) ); ?></span>
									<?php
								}
							}
						}
					...
					<li style="color: #333333; font-size: 12px; overflow: hidden;">
						...
						if ( is_array( $extra_child ) ) {
							foreach ( $extra_child as $item_extra_child ) {
								if ( $item['room_child'] == $item_extra_child['number'] ) {
									?>
									<span class="apb-room-seleted_date">
										<?php printf( esc_html__( '%s Adult', 'awebooking' ), absint( $item_extra_adult['number'] ) ); ?> + <?php printf( esc_html__( '%s/night', 'awebooking' ), wp_kses_post( AWE_function::apb_price( $item_extra_adult['price'] ) ) ); ?>
									</span>
									<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount"><?php echo wp_kses_post( AWE_function::apb_price( $item_extra_child['price'] ) ) ?> × <?php echo absint( count( $range_date ) ); ?></span>
									<?php
								}
							}
						}
			...
			<?php if ( ! empty( $item['package_data'] ) ) : ?>
				<h6 style="color: #333333; display: inline-block; font-size: 14px; font-weight: bold; line-height: 1.428em; margin: 0 10px 0 0; text-transform: uppercase;">
					<?php esc_html_e( 'Package', 'awebooking' ); ?>
				</h6>
				<ul style="list-style: outside none none; margin-bottom: 0; margin-top: 5px; padding-bottom: 2px; padding-left: 0;">
					<?php
					foreach ( maybe_unserialize( $item['package_data'] ) as $info_package ) :
		...
		<div class="apb-room-seleted_total-room" style="color:#333333;font-size:14px;font-weight:bold;border-top: 1px solid #e4e4e4;padding-top: 15px;">
			<?php esc_html_e( 'Total Room', 'awebooking' ); ?> <?php echo esc_attr( $room_num ) ?><span class="apb-amount" style="color:#46598b;float:right;font-weight:bold;"><?php echo wp_kses_post( AWE_function::apb_price( $item['total_price'] ) ); ?></span>
		</div>
	</div>
	<!-- END / ITEM -->
<?php endforeach; ?>
