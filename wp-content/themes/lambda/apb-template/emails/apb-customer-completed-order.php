...
$config_email = get_option( 'apb_mail_complete' );
						...
						if ( is_array( $extra_adult ) ) {
							foreach ( $extra_adult as $item_extra_adult ) {
								if ( $item['room_adult'] == $item_extra_adult['number'] ) {
									...
									<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount" class="apb-amount"><?php echo wp_kses_post( AWE_function::apb_price( $item_extra_adult['price'] ) ) ?> × <?php echo absint( count( $range_date ) - 1 ); ?></span>
									<?php
								}
							}
						}
						...
						if ( is_array( $extra_child ) ) {
							foreach ( $extra_child as $item_extra_child ) {
								if ( $item['room_child'] == $item_extra_child['number'] ) {
									...
									<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount"><?php echo wp_kses_post( AWE_function::apb_price( $item_extra_child['price'] ) ) ?> × <?php echo absint( count( $range_date ) - 1 ); ?></span>
									<?php
								}
							}
						}
