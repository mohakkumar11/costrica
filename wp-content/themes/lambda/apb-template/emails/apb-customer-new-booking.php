...
$config_email = get_option( 'apb_mail_new_booking' );
...
<div style="border-bottom: 1px solid #e4e4e4; padding: 20px 20px 15px;" class="apb-room-selected_item">
						...
						foreach ( $extra_adult as $item_extra_adult ) {
							if ( $item['adult'] == $item_extra_adult['number'] ) {
								...
								<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount" class="apb-amount">
									<?php echo wp_kses_post( AWE_function::apb_price( $item_extra_adult['price'] ) ) ?> × <?php echo absint( count( $range_date ) - 1 ); ?>
								</span>
								<?php
							}
						}
						...
						foreach ( $extra_child as $item_extra_child ) {
							if ( $item['child'] == $item_extra_child['number'] ) {
								...
								<span style="float: right; font-weight: bold; text-transform: uppercase;" class="apb-amount">
									<?php echo wp_kses_post( AWE_function::apb_price( $item_extra_child['price'] ) ) ?> × <?php echo absint( count( $range_date ) - 1 ); ?>
								</span>
								<?php
							}
						}
	...
	<div class="apb-room-seleted_total-room" style="color:#333333;font-size:14px;font-weight:bold;border-top: 1px solid #e4e4e4;padding-top: 15px;">
		<?php esc_html_e( 'Total Room', 'awebooking' ); ?><?php echo esc_attr( $room_num ) ?><span class="apb-amount" style="color:#46598b;float:right;font-weight:bold;"><?php echo AWE_function::apb_price( $price ); ?></span>
	</div>
</div>
<!-- END / ITEM -->
