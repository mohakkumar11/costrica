...
class Form {
	...
	private function build_select($id,$data) {
		$html ="<select name='{$id}' class='form-control' id='{$id}'>" . $this->build_select_options($data['define']['layout']['options'],$data['value'],!empty($data['define']['layout']['single'])) . "</select></li>";
		return $html;
	}
	...
	private function build_checkbox($id,$data) {
		$html = "<input type='checkbox' class='form-control' name='{$name}' id='{$id}' value='1'";
	...
	private function build_textarea($id,$data) {
		$html = "<textarea name='{$id}'  class='form-control' id='{$id}'>" . $this->escape($data['value'])  . "</textarea>";
		return $html;
	}
	...
	private function build_radio_group($data,$id,$sel) {
		...
		foreach($data as $key => $val) {
			$html .= "<input type='radio' class='form-control' name='{$id}' value='" . $this->escape($val) . "'";
			if($sel === $key) {
